  ...
  <div class="register-form-container">
    <form id="registerForm" @submit="handleSubmit" novalidate autocomplete="off">
      ...
      <div class="form-group">
       ...
       <p class="error-mess" v-if="errorObj.confirmErr.length > 0">{{ errorObj.passErr[0] }}</p>
      </div>
      ...
      <div class="form-group">
       ...
       class="form-control" @click="availableTime()" v-model="registerObj.birth" />
  ...
  methods: {
    ...
    checkForm: function () {
     ...
     if (!this.registerObj.name) {
      ...
      if (!/^[a-z0-9._%+-]+\.[a-z]{2,3}$/.test(this.registerObj.email)) {
     ...
     if (!this.registerObj.pass) {
      ...
      if (this.registerObj.pass.length < 8) {
        this.errorObj.passErr.push('Password must be more or equal 8 characters');
      }
     }
     ...
     if (!this.registerObj.phone) {
      ...
      this.errorObj.phoneErr.push('Phone numbers must start woth 84');
     }
